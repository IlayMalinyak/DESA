...
from eigenspace_analysis import *
...
def plot_quantile_predictions(df_path, merge_cat_path=None, left_on='obsid',
                              ...
                              color=None, snr_cut=None, snr_max=np.inf, restrict_teff=False,
                              suffix='', return_figs=False):
    # [All your existing preprocessing code - unchanged]
    df = pd.read_csv(df_path)
    if 'obsid' in df.columns:
        if df['obsid'].dtype == 'object':
    ...
    figures = []  # Store figures if return_figs=True

    for col in target_cols:
        # [All your existing column processing code - unchanged]
        if 'teff' in col.lower():
            mult_fact = 5778
        elif 'vsini' in col.lower():
            mult_fact = 100
        ...
        # Get the figure from plot_quantile_property
        fig = plot_quantile_property(df_col, col, label, color, fraction,
                                     modal_type, mult_fact, snr_str,
                                     os.path.dirname(df_path),
                                     scale_target=scale_target, suffix=suffix)

        if return_figs:
            figures.append(fig)
        else:
            # Original behavior - save and show
            plt.savefig(os.path.join(os.path.dirname(df_path),
                                     f'{label}_targets_with_confidence_cut_{snr_str}{suffix}.png'))
            plt.show()
...
def plot_quantile_property(df_col, target_col, label,
                           ...
                           mult_fact, snr_cut, save_dir,
                           scale_target=True, suffix=''):
    """
    Modified to return figure instead of showing/saving it
    """
    ...
    preds_lower_80 = df_col[f'pred_{label}_q0.100'].values * mult_fact
    preds_upper_80 = df_col[f'pred_{label}_q0.900'].values * mult_fact
    preds_lower_50 = df_col[f'pred_{label}_q0.250'].values * mult_fact
    preds_upper_50 = df_col[f'pred_{label}_q0.750'].values * mult_fact
    ...
    r_squared = r2_score(targets, preds_median)
    acc_10p = (np.abs(targets - preds_median) < (np.abs(targets) * 0.1)).sum() / len(targets)
    coverage_80 = ((targets >= preds_lower_80) & (targets <= preds_upper_80)).sum() / len(targets)
    coverage_50 = ((targets >= preds_lower_50) & (targets <= preds_upper_50)).sum() / len(targets)

    print("metrics:\n ", "MAE:", avg_err, "RMSE:", rms, "R2:", r_squared, "ACC:", acc_10p,
          "80% coverage:", coverage_80, "50% coverage:", coverage_50)

    # Create figure
    sort_idx = np.argsort(preds_median)
    preds_median = preds_median[sort_idx]
    preds_lower_80 = preds_lower_80[sort_idx]
    preds_upper_80 = preds_upper_80[sort_idx]
    preds_lower_50 = preds_lower_50[sort_idx]
    preds_upper_50 = preds_upper_50[sort_idx]
    targets = targets[sort_idx]

    if color is None:
        fig, ax = plt.subplots(figsize=(10, 10))
        ax.scatter(preds_median, targets, alpha=0.5, s=10, color='sandybrown')

        ax.plot(preds_median,
                savgol(preds_lower_50, 20, polyorder=1),
                alpha=0.3,
                color='dimgray')
        ax.plot(preds_median,
                savgol(preds_upper_50, 20, polyorder=1),
                alpha=0.3,
                color='dimgray',
                label='50% confidence interval')
        ax.plot(preds_median,
                savgol(preds_lower_80, 20, polyorder=1),
                alpha=0.3,
                color='darkblue')
        ax.plot(preds_median,
                savgol(preds_upper_80, 20, polyorder=1),
                alpha=0.3,
                color='darkblue',
                label='80% confidence interval')

        # Set equal aspect and limits
        data_min = min(np.min(preds_median), np.min(targets))
        data_max = max(np.max(preds_median), np.max(targets))
        ...
        # Diagonal line
        ax.plot([data_min, data_max], [data_min, data_max], 'k--', alpha=0.8)

        # Labels and title
        ax.set_xlabel(f'Predicted {latex_names[label]} ({units[label]})')
        ax.set_ylabel(f'Target {latex_names[label]} ({units[label]})')
        ax.set_title(f'MAE: {avg_err:.3f} ({units[label]})')
        ax.legend(loc='best')

        return fig  # Return the figure instead of showing/saving

    else:
        # Handle color cases if needed - return figure here too
        pass




def create_quantile_plot(plot_data, ax):
    """Helper function to create the actual plot on given axis"""

    # Scatter plot
    ax.scatter(plot_data['preds_median'], plot_data['targets'],
               alpha=0.5, s=10, color='sandybrown')

    # Confidence intervals with smoothing
    from scipy.signal import savgol_filter

    ax.plot(plot_data['preds_median_sorted'],
            savgol_filter(plot_data['preds_lower_50_sorted'],
                          min(20, len(plot_data['preds_lower_50_sorted']) // 5),
                          polyorder=1),
            alpha=0.3, color='dimgray')

    ax.plot(plot_data['preds_median_sorted'],
            savgol_filter(plot_data['preds_upper_50_sorted'],
                          min(20, len(plot_data['preds_upper_50_sorted']) // 5),
                          polyorder=1),
            alpha=0.3, color='dimgray', label='50% confidence interval')

    ax.plot(plot_data['preds_median_sorted'],
            savgol_filter(plot_data['preds_lower_80_sorted'],
                          min(20, len(plot_data['preds_lower_80_sorted']) // 5),
                          polyorder=1),
            alpha=0.3, color='darkblue')

    ax.plot(plot_data['preds_median_sorted'],
            savgol_filter(plot_data['preds_upper_80_sorted'],
                          min(20, len(plot_data['preds_upper_80_sorted']) // 5),
                          polyorder=1),
            alpha=0.3, color='darkblue', label='80% confidence interval')

    # Set equal aspect and limits
    data_min = min(np.min(plot_data['preds_median']), np.min(plot_data['targets']))
    data_max = max(np.max(plot_data['preds_median']), np.max(plot_data['targets']))
    ...
    ax.set_xlabel(f'Predicted {latex_names[label]} ({units[label]})')
    ax.set_ylabel(f'Target {latex_names[label]} ({units[label]})')
    ax.set_title(f'MAE: {plot_data["avg_err"]:.3f} ({units[label]})')

    # Legend
    ax.legend(loc='best')


def get_plot_data_for_targets(df_path, merge_cat_path=None, left_on='obsid',
                              right_on='combined_obsid', target_prefix=None,
                              color=None, snr_cut=None, snr_max=np.inf,
                              restrict_teff=False, suffix=''):
    """
    Modified to return plot data without creating figures
    """
    df = pd.read_csv(df_path)
    ...
    for col in target_cols:
        # Your existing column processing code
        if 'teff' in col.lower():
            mult_fact = 5778
        elif 'vsini' in col.lower():
            mult_fact = 100
        ...
        # Get plot data without plotting
        plot_data = plot_quantile_property(df_col, col, label, color, fraction,
                                           modal_type, mult_fact, snr_str,
                                           os.path.dirname(df_path),
                                           scale_target=scale_target, suffix=suffix,
                                           ax=None)  # Don't plot, just return data
...
def scatter_mae_vs_snr(df_path, target_col, snr_cuts):
    ref_cat = pd.read_csv(lamost_apogee_path)
    ref_cat = ref_cat.dropna(subset=['APOGEE_TEFF', 'APOGEE_LOGG', 'APOGEE_FEH'])
    ref_cat = ref_cat[ref_cat['APOGEE_TEFF'] < 7500]
    df = pd.read_csv(df_path)

    df['obsid'] = df['obsid'].apply(lambda x: int(x.split('.')[0]))
    df = df.merge(ref_cat, left_on='obsid', right_on='combined_obsid')
    df['snrg'] *= 1000
    target_col = f'target_{target_col}'
    avg_maes = []
    std_maes = []
    for snr in snr_cuts:
        df_cut = df[df['snrg'] > snr]
        if 'teff' in target_col.lower():
            mult_fact = 5778
        elif 'vsini' in target_col.lower():
            mult_fact = 100
        else:
            mult_fact = 1

        # Filter data
        if 'feh' not in target_col.lower():
            df_col = df_cut[df_cut[target_col] > 0]
        else:
            df_col = df_cut

        label = target_col.split('_')[-1]
        targets = df_col[f'APOGEE_{label.upper()}'].values
        preds_median = df_col[f'pred_{label}_q0.500'].values * mult_fact
        mae = np.abs(targets - preds_median)
        print(f'snr: {snr}')
        print(f'mae: {mae.mean()}')
        print(f'std: {mae.std()}')
        avg_maes.append(mae.mean())
        std_maes.append(mae.std())
    plt.scatter(snr_cuts, avg_maes, s=100)
    # plt.scatter(snr_cuts, std_maes, marker='*')
    plt.show()
...
def create_combined_plot(df_path):
    """
    Create 2x3 subplot with all 6 plots
    """
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))

    # Get all 6 individual plots
    plots = []

    # First get regular plots (top row)
    for cut in [None]:
        snr_max = np.inf
        regular_plots = plot_quantile_predictions(df_path, merge_cat_path=lamost_apogee_path,
                                                  snr_cut=cut, snr_max=snr_max, target_prefix=None,
                                                  restrict_teff=False, suffix='', return_figs=True)
        plots.extend(regular_plots)

        # Then get APOGEE plots (bottom row)
        apogee_plots = plot_quantile_predictions(df_path, merge_cat_path=lamost_apogee_path,
                                                 snr_cut=cut, snr_max=snr_max, target_prefix='APOGEE',
                                                 restrict_teff=True, suffix='_apogee_restricted', return_figs=True)
        plots.extend(apogee_plots)

    # Copy each plot to the combined subplot
    for i, plot_fig in enumerate(plots[:6]):  # Only first 6 plots
        row = i // 3
        col = i % 3

        # Copy the plot content to the subplot
        plot_ax = plot_fig.gca()  # Get the axis from the individual plot

        # Copy all the content from individual plot to subplot
        for line in plot_ax.get_lines():
            axes[row, col].plot(line.get_xdata(), line.get_ydata(),
        ...
        for collection in plot_ax.collections:
            if hasattr(collection, 'get_offsets'):  # Scatter plots
                axes[row, col].scatter(collection.get_offsets()[:, 0], collection.get_offsets()[:, 1],
                                       color=collection.get_facecolors()[0], alpha=collection.get_alpha(),
                                       s=collection.get_sizes())

        # Copy labels and title
        axes[row, col].set_xlabel(plot_ax.get_xlabel())
        axes[row, col].set_ylabel(plot_ax.get_ylabel())
        axes[row, col].set_title(plot_ax.get_title())
        axes[row, col].set_xlim(plot_ax.get_xlim())
        axes[row, col].set_ylim(plot_ax.get_ylim())
        axes[row, col].set_aspect('equal')
        axes[row, col].legend()

        plt.close(plot_fig)  # Close the individual plot

    plt.tight_layout()
    plt.savefig('combined_quantile_plots.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plots_for_poster():
    ...
    # create_combined_plot(spec_df_path)
    # for cut in [None]:
    #     snr_max = np.inf
    #     plot_quantile_predictions(spec_df_path, merge_cat_path=lamost_apogee_path,
    #                               snr_cut=cut, snr_max=snr_max, target_prefix=None, restrict_teff=False, suffix='')
    #     plot_quantile_predictions(spec_df_path, merge_cat_path=lamost_apogee_path,
    #                               snr_cut=cut, snr_max=snr_max, target_prefix='APOGEE', restrict_teff=True, suffix='_apogee_restricted')
...
def plots_comparisons():
    ...
    lr_compare = os.path.join(root_dir, 'lightspec', 'lightspec_2025-05-14', 'compare_fewshot')
    zero_compare = os.path.join(root_dir, 'lightspec', 'lightspec_2025-05-14', 'compare_zeroshot')

    compare_umaps(umap_compare, ['projections', 'features', 'features', 'features', 'features'], 'flag_CMD_numeric')
    # compare_umaps(umap_compare, ['projections', 'features', 'features', 'features', 'features'], 'Lstar')

    # compare_zero_shot(zero_compare, ['umap_coords']*5)
    # compare_zero_shot(zero_compare, ['projections', 'features', 'features', 'features', 'features'])

    # compare_lr(lr_compare, ['projections', 'features', 'features','features', 'features'], ['BPmRP_0', 'M_G_0'])
    # compare_regression_experiments(age_compare_dir,  ['final_age_norm', 'age_error_norm'], color_col='Teff')
    # for file in os.listdir(age_compare_dir):
    #     if file.endswith('.csv'):
    #         print(file)
    #         if 'dual_former' in file:
    #             file_name = file.replace('predictions_', '').replace('.csv', '')
    #             plot_regression_results(age_compare_dir, file_name,
    #                                     ['final_age_norm', 'age_error_norm'],
    #                                     color_col='Teff')
    # compare_cls_experiments(nss_compare_dir)
    # plot_classification_results(nss_compare_dir, 'nss_lightspec_finetune_dual_former', snr_cut=20)
...
if __name__ == '__main__':
    ...
    age_compare_dir = os.path.join(root_dir, 'finetune', 'age_compare')
    umap_compare = os.path.join(root_dir, 'lightspec', 'lightspec_2025-05-14', 'compare_umaps')
    finetune_age_dir = os.path.join(root_dir, 'finetune', 'age_finetune_2025-05-26')
    ...
    compare_pair_distances(umap_compare)
    # umap_desa = os.path.join(umap_compare, 'DESA (ours)')
    # for file in os.listdir(umap_desa):
    #     if file.endswith('.csv'):
    #         plot_umap_with_pairs(umap_desa,
    #                              file.replace('preds_', '').replace('.csv', ''),
    #                              umap_input='projections')
    exit()

    # scatter_mae_vs_snr(spec_df_path, 'teff', [0,5,8,10,20,30,40,50,80.100])
    # exit()
    ...
    #
    # exit()
    plot_training_loss(os.path.join(root_dir, 'spec', 'spec_decode2_2025-02-11'))
